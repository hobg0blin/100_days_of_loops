...
def makeDroste(baseImage, stopAfter=20):
    ...
    print('base image mode: ', baseImage.mode)
    ...
    print('magenta width: ', magentaLeft, magentaRight)
    print('magenta height: ', magentaTop, magentaBottom)
    ...
    for x in range(magentaLeft, magentaRight + 1):
        for y in range(magentaTop, magentaBottom + 1):
            if baseImage.getpixel((x, y)) == magentaColor:
                print('should be putting pixel:', x, y)
...
recursiveImage = makeDroste('droste.png', 20)
recursiveImage.save('droste-recursive.png')
recursiveImage.show()
