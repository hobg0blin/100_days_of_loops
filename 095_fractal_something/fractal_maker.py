...
if (args.fractal):
    print("Drawing fracal: ", args.fractal)
...
# Draw the three sides of the triangle:
    for i in range(3):
        turtle.forward(size)
        turtle.right(120)
def drawCircleOutline(size, depth):
    size = int(size)
    turtle.penup()
    turtle.left(90)
    turtle.pendown()
...
# change is added to the turtle's current heading.
        sizeCh = spec.get('sizeChange', 1.0) * random.randrange(1, 4)
...
if DRAW_FRACTAL == 1:
# Four Corners:
    drawFractal(drawCircleOutline, 350,
...
# Spiral Squares:
    drawFractal(drawCircleOutline, 600, [{'sizeChange': 0.95,
'angleChange': 7}], 50)
elif DRAW_FRACTAL == 3:
...
# Sierpiński Triangle:
    toMid = math.sqrt(3) / 6
    drawFractal(drawCircleOutline, 600,
...
# Wave:
    drawFractal(drawCircleOutline, 280,
    [{'sizeChange': 0.5, 'xChange': -0.5, 'yChange': 0.5},
...
# Snowflake:
    drawFractal(drawTriangleOutline, 100,
